...
from utils import one_hot_encode,dice_coef_loss,dice_coeff,f1_score

model_predict = load_model('survival_pred.h5',custom_objects={'dice_coef_loss':dice_coef_loss, 'f1_score':f1_score})
...
'''
  ...
  pred1 = np.argmax(pred.reshape(-1,5)[:,0:5],axis = 1)
  y2 = np.argmax(y_to.reshape(-1,5)[:,0:5],axis = 1)
